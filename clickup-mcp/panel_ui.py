...
document.addEventListener("click", function (ev) {
  ...
  if (ev.target.closest("#empreview")) {
    /* Renders the exact HTML the send builds in a sandboxed frame, so what
       you see is what goes out, plain mode included. It also runs the same
       body checks the send does, so a message that would be refused shows
       the reason here instead of at send time. */
    var pvhtml = emBodyHtml(), pvsubj = $("#emsubj").value.trim();
    var pvplain = !!($("#emplain") && $("#emplain").checked);
    var mp = $("#emmsg"), pane = $("#empvpane");
    if (!pvhtml) { mp.textContent = "write the message first"; return; }
    fetch("/email", { method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "preview", subject: pvsubj, html: pvhtml, plain: pvplain }) })
      .then(function (r) { return r.json(); })
      .then(function (d) {
  ...
  if (ev.target.closest("#emsend")) {
    ...
    var plainMode = !!($("#emplain") && $("#emplain").checked);
    ...
    fetch("/email", { method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "send", segment: seg, expect: count,
                             confirm_count: confirmCount, subject: subj2, html: html2, plain: plainMode }) })
      .then(function (r) { return r.json(); })
      .then(function (d) {
...
def _email_card(d: dict) -> str:
    ...
    return (
        ...
        f'vault.</p>'
        f'<p class="note">To stop emailing someone, add their address to '
        f'<code>email-suppress.txt</code> in that same private folder, one per '
        f'line; they drop out of every audience and are never mailed again.</p>'
        ...
        f'<label class="em-plain" title="Send the message exactly as written: '
        f'no logo, no footer, no unsubscribe header, so it reads as a personal '
        f'note and lands in the primary inbox rather than promotions. The body '
        f'must carry its own unsubscribe line.">'
        f'<input type="checkbox" id="emplain"> Plain mode</label>'
        ...
        f'<div class="em-pv-head">This is exactly what sends '
        f'&middot; Subject: <b id="empvsubj"></b></div>'
